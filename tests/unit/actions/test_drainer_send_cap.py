...
# Auto-seed one before each mint so these Story-4-6 send-cap tests keep their
# setup intent (gate coverage lives in test_mint_requires_user_confirmation.py).
...
def _auto_confirm_grants(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "tests.unit.actions.test_drainer_send_cap.mint_grant", _mint_with_confirmation
    )
