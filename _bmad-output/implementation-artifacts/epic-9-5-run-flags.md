...
- **(d) Evaluator model IDs (reproducibility):** primary `claude-opus-4-7`, secondary `claude-haiku-4-5-20251001`. **Deviation from story file:** the story spec'd `--secondary-model claude-sonnet-4-5`, which has NO registered adapter (`mailbot_api/router/registry.py` registers only qwen + haiku-4-5-20251001 + opus-4-7) — Run 1 (09:13Z; CLI dispatch in the walk-evidence sense, NOT this file's halt-event "Run N" headings — see item (e)'s disambiguation note *(tag added 2026-07-04 CR-of-CR B-P9)*) failed all 40 secondary dispatches pre-API (latency 0ms, $0) and exited via the zero-pairs sentinel (exit 2, no file written, guard behaved to spec). Adam-decided the haiku re-run, which matches this file's own line 52 original secondary-evaluator description. Filed as F-SECONDARY-MODEL-UNREGISTERED (create-story authored a CLI against an unregistered model).
- **(e) Evidence + artifact:** `_bmad-output/implementation-artifacts/9.5.4-walk-evidence.md` (full capture: both runs verbatim, per-anchor disagreement table, triage, findings, spend). Baseline artifact host-side at `evals/anchor_baselines/v1-FAILED-CALIBRATION.json` (gitignored operator-state; BaselineSnapshot-validated; α consistent with stdout after the CLI's `:.4f` formatting — raw JSON `0.3489827926151641` → `0.3490`; "byte-identical" is unachievable by design and was a misstatement, corrected 2026-07-04 post-commit CR P3). **Run-numbering note (post-commit CR P11):** "Run 1/Run 2" in the walk-evidence file = the two CLI dispatches (both 2026-07-04); "Story 9.5.4 Run 1/Run 2" headings in THIS file = dev-agent halt events (2026-07-03/04) — different events.
...
## Story 9.5.4 honest α re-run — 2026-07-04 — PATH B SIGNED, STORY DONE

*(Run-numbering: this section documents the third CLI dispatch — the honest re-run after the F-ANCHOR-ANSWER-KEY-LEAK fix — executed in the Adam-invoked "run the Task 6 contract" session, same day as the walk. "Run 1/Run 2" headings above = dev-agent halt events; the dispatch history is Run 1 zero-pairs + Run 2 contaminated + this honest run.)*

- **(a) Clause closed:** Epic 9 done-flip **clause 9 closes at L3** — honest α measured on uncontaminated prompts + Adam-signed Path B accept-with-rationale per Story 9-11 AC-7 OR-branch (retro entry: `epic-9-5-retro-2026-07-04.md`). **Clause 10 ALSO closes at L3** via Adam-amended AC-7 criteria (honest `v1-FAILED-CALIBRATION.json`, BaselineSnapshot-validated, serves as the v1 anchor-drift baseline; canonical `v1.json` + α≥0.6 = v2 gates).
- **(b) Story 9.5.3 AC-7 cross-discharge:** **YES — via the Path B amendment** (not via canonical v1.json, which untrusted never writes). sprint-status line-262 suffix appended.
- **(c) Verdict verbatim (CLI stdout):** `VERDICT=untrusted alpha=0.3490 (threshold < 0.60). Wrote FAILED-CALIBRATION payload to /data/anchor_baselines/v1-FAILED-CALIBRATION.json. Canonical baseline NOT updated.` — exit 2; `n_anchors_paired=40` per validated BaselineSnapshot. **Load-bearing: raw α float and all 40 score pairs are byte-identical to the contaminated Run 2** — the leak was not driving agreement; the α failure is F-HAIKU-SCALE-COMPRESSION (10/11 non-zero deltas) + the synthetic cyclic anchor-design caveat (retro seed).
- **(d) Evaluator model IDs:** primary `claude-opus-4-7`, secondary `claude-haiku-4-5-20251001` — Run-2 command verbatim; only the underlying code differed (leave-one-out leak fix + CR2026-07-04B-W1 SYSTEM count-wording fix, both docker-cp-verified live pre-dispatch; runner.py 9.5.3 hotfix found dropped in-container and re-injected, not load-bearing here).
- **(e) Evidence + artifact:** `9.5.4-walk-evidence.md` § "Honest α re-run". Host artifacts: `evals/anchor_baselines/v1-FAILED-CALIBRATION.json` (honest) + `archived-v1-FAILED-CALIBRATION.2026-07-04-answer-key-contaminated.json` (contaminated, archived pre-dispatch to keep the both-files tripwire meaningful). Both gitignored operator-state.
- **(f) Path decision:** **Path B accept-with-rationale, Adam-signed 2026-07-04 at walk close** (per the HYBRID pre-decision: honest α < 0.6 → Path B on an uncontaminated number). Story 9.5.4 → `done` per Task 6 status rule; Story 9.5.5 gate → `Path-B-accept-with-rationale` (unblocked).
- **(g) Spend:** pre-flight estimate $9.14 placeholder (cost-gate abort method, zero dispatch); billed $21.8502 placeholder (opus $20.6396/40ok + haiku $1.1511/39ok + $0.0595/1 retry_recovered; 0 cache hits) — estimator under-billed 2.4× reproducing post-commit-CR P5. **Real per Console: ~$18 pre → ~$26 post (Adam-read approximations) ≈ $8 real vs ~$8 Adam-authorized — no overshoot.** The ~$18 pre-reading also captured the contaminated walk's missing post-reading (≈$4.5 real vs $13.46 baseline) → **post-commit-CR D1 spend reconciliation CLOSED.** Monthly ~$26 of $35 cap.

## Story 9.5.5 autonomous run (2026-07-04)

- **Run mode:** /autonomous-story-run 9.5.5 (first Epic 9.5 story compatible with autonomous execution — no run-mode binding, $0 spend, route (b) pre-resolved).
- **Close shape:** route (b) no-change — `router/policy.yaml` version `policy-v0-2026-06-01` → `policy-v1-2026-07-04`; diff = version line + comment blocks only (verified via git diff, zero routing mutations); +5 net tests (1685+2+3 from 1680+2+3 baseline); 4 gates green.
- **MANDATORY-CR FULL scope (AC-6):** round 1 under claude-sonnet-5 (dev claude-fable-5; sonnet-4-6→sonnet-5 substitution per 9.5.4 D2 blessing) — 3 findings: CR-F1 MEDIUM applied (model pins on tone_style_mirror/multi_turn_refinement in routing-unchanged guard), CR-F3 LOW applied (F-COHORT-KEY-LEGACY-SCORE-CONFLATION named in policy.yaml draft_reply comment + retro entry reason 3), CR-F2 LOW **deferred**: AC-5 E2E dispatches coarse_class not draft_reply — rationale: cohort `router_policy_version` component is task-agnostic by construction (`_read_policy_versions()` snapshots `PolicyTable.version` once per run); a draft_reply scripted-adapter fixture would duplicate the 9-8 precedent surface for no added coverage of the component under test. Revisit if per-task policy-version plumbing is ever introduced. 2/2 actionable = 100% applied-rate; no round 2 (fixes trivial).
- **PENDING-ADAM-SIGNATURE (AC-3):** the route-(b) retro entry in `epic-9-5-retro-2026-07-04.md` § "policy.yaml v0→v1" is drafted but unsigned — the autonomous run may not self-sign. Phase 3.5 PASS verdict = signature granted (entry then edited to Adam-signed); FAIL = story flips back to in-progress.
- **WARNING (pre-existing, out of scope):** 2 ruff T201 errors in untracked `scratch/walk_bootstrap.py` (9.5.3/9.5.4 walk leftover) — full-repo `ruff check .` is red on that file alone; story surface clean. Recommend deleting or .gitignoring scratch/ (it is untracked; never staged by this run).
- **Epic-close note:** with 9.5.5 done, all 5 Epic 9.5 stories are done. Epic done-flip NOT performed by this run (single-story scope); epic-9-5-retrospective is the natural next step, where the pending signature + this entry consolidate.
