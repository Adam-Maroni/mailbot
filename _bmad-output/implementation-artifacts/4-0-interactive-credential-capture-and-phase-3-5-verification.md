...
| 2026-06-04 | Story 6-6.5 **third-pass agent-driven MCP walk** — F17 RESOLVED by Story 6-11 (worker-process pipeline-runtime init gap closed). **Drainer e2e + real Graph write-back PROVEN at dispatch layer**: drainer claimed `pending_actions.id=1`, `OutlookGraphWriteAdapter` dispatched `POST /me/messages/{id}/reply` to Microsoft Graph, `budget_consumed=1` set, Tier-3 urgent notification fired. Graph returned **HTTP 401 Unauthorized** because of new **F23** (Microsoft refresh token rejected — OPERATIONAL, not code; expired refresh token, 9+ hours of `oauth.refresh.failed`). **20-send/day cap remains AGENT-SURROGATE-PASS** (3/3 cap-check scenarios verified previously; live full-walk still F23-gated). Two code defects discovered + inline-fixed during the walk: F19 (Anthropic temperature deprecated on Opus 4.7) + F22 (no pending_grant promotion on mint_grant). Four follow-up stories filed: 6-12 (F19 CR + tests), 6-13 (F22 CR + tests), 6-14 (F21 Haiku summary_short investigation), **6-15 (F23 re-auth runbook — BLOCKS final 3-CP closure)**. 3 deferred CPs status: drainer e2e + real Graph write-back **WIRING-PROVEN-PENDING-F23** (only "real send completes" leg un-verified); 20-send/day cap live **AGENT-SURROGATE-PASS retained, full live walk pending F23**. NOT silent close — explicit gate on Story 6-15. See `epic-6-run-flags.md § Story 6-6.5 walk record § Third pass (2026-06-04)`. |
