...
- **Story 5-9 capstone carry-forward** (drainer e2e + real Graph write-back + 20-send/day cap live) — Section A of Story 6-6.5 walk **PASS 2026-06-04** (wiring proven live: scheduler ticking, drainer ticking, MCP discovery green, sync healthy). Section B walked in 3 passes: pass-1 PARTIAL-BLOCKED-by-F17 (since RESOLVED by Story 6-11); pass-2 N/A; **pass-3 third-pass agent-driven MCP walk 2026-06-04** — CP-A PASS-WITH-FINDINGS (full wiring proven end-to-end through Microsoft Graph dispatch; final recipient-inbox leg F23-blocked = expired refresh token, operational); CP-B BLOCKED-by-F23 (same Graph 401 path); CP-C PASS (confidential refusal verified live: `state=confidential_refused`, defender_message canonical, router_calls delta=0); CP-D AGENT-SURROGATE-PASS retained. Two code defects inline-fixed during the third-pass walk (F19 Anthropic temperature deprecated on Opus 4.7 + F22 no pending_grant promotion on mint_grant) and live-verified; one code defect filed for follow-up (F21 Haiku summary_short outcome=failed despite billing); one operational blocker filed for follow-up (F23 Microsoft refresh token rejected). Four follow-up stories filed: 6-12 (F19 CR + tests), 6-13 (F22 CR + tests), 6-14 (F21 investigation), 6-15 (F23 re-auth runbook). **The wiring CPs (drainer e2e + real Graph write-back) are PROVEN at the dispatch layer** — drainer claimed, adapter dispatched, Graph endpoint hit, budget_consumed=1 set, Tier-3 urgent notification fired. Only un-verified leg is "real send completes" which is F23-gated. Capstone carry-forward stays **ADAM-Section-B-PARTIAL-WIRING-PROVEN-PENDING-F23** until Story 6-15 closes + Adam re-auths + CP-A/B re-walks with recipient-inbox verification. See `epic-6-run-flags.md § Story 6-6.5 walk record § Third pass (2026-06-04)`. NOT silent close — explicit gate on 6-15.
