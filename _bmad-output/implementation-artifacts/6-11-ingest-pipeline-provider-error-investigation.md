...
- [x] [Review][Patch] NFR-SEC-4: `error_message` log field can leak unsanitized exception text from classifier-side `RouterError` constructors [mailbot_api/ingest/pipeline.py:352] — APPLIED 2026-06-04 via defense-in-depth wrap at the pipeline boundary using `observability.logging.sanitize` (string-level redaction). Import added at line 49; field wrapped at lines 352-358; comment updated to explain the contract. Surgical fix per spec (1 file, 7 net lines). Gates: ruff clean, mypy --strict clean, boundary checker silent, 11 affected integration tests pass. — Acceptance Auditor finding. The dev's inline comment at `pipeline.py:348-351` claims all PROVIDER_ERROR sites sanitize via `router.py`, but `mailbot_api/sensitivity/classifier.py:97-101` constructs `RouterError(message=f"sensitivity classifier could not read policy snapshot: {exc}", ...)` with NO `sanitize_error()` call. Lines 104, 118, 159 are similar but use static strings (lower risk). Today's surfaced message ("policy not loaded …") is non-sensitive infrastructure text, so no actual leak occurred. But future drift where the wrapped `RuntimeError` carries a path or secret (e.g., yaml parse error referencing `/path/to/something.env`) would leak directly into the warning-level log. Fix options: (a) wrap `{exc}` → `{sanitize_error(exc)}` at `classifier.py:99` (and 104, 118, 159 for defense-in-depth across all 4 constructor sites), OR (b) at `pipeline.py:352` use the string-level `observability.logging.sanitize` on `sensitivity_result.error.message` before logging.
- [x] [Review][Defer] Regression test calls `init_pipeline_runtime` directly rather than exercising `_worker_main` wiring [tests/integration/test_worker_pipeline_runtime_init.py:135,175] — deferred, test-strength gap not a code defect (TRIPLE-source consensus from Blind+Edge+Auditor; logged to deferred-work.md)
- [x] [Review][Defer] `init_default_adapters()` nested inside `if not skip_policy:` — `SKIP_POLICY=1, SKIP_PATTERNS=0` silently skips adapter registration [mailbot_api/ingest/pipeline.py:735] — deferred, faithful mirror of `main.py:162` pre-existing pattern, out of scope per "surgical fix, not a refactor"
- [x] [Review][Defer] Brittle boolean parsing of `MAILBOT_SKIP_*` flags — `"true"`, `"True"`, `"1 "`, `"yes"` all silently fail to skip [mailbot_api/ingest/pipeline.py:714-715] — deferred, project-wide pattern across all skip flags (needs centralized `_truthy()` helper sweep)
- [x] [Review][Defer] AC-4 backlog drain verified over 2 ticks instead of spec-mandated 6 — deferred, dev acknowledged in Task 4 (strictly-decreasing trend met; backpressure flip is a time-dependent observation requiring ~9h of drain)
