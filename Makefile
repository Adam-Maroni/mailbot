...
# docker save + scp + docker load + rolling restart on VPS. Full body lands in epic 6 (deploy.sh).
deploy:
	@echo "[deploy] placeholder — full body in epic 6 (scripts/deploy.sh)"
...
# Nightly SQLite .backup + config tarball. Full body lands in epic 6 (scripts/backup.sh).
backup:
	@echo "[backup] placeholder — full body in epic 6"
